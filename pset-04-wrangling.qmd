...
request <- request(
  paste0(
    ...
    "get=POP_2020,POP_2021,NAME&for=state:*&key=", census_key
  )
)
...
# data converted to a maatrix
population <- resp_body_json(response,simplifyVector = TRUE, simplifyDataFrame = TRUE)
...
population |> 
  # reorder state
  ggplot(aes(x = reorder(state_name,-pop), y = pop/1000000)) +
  # use geom_col to plot barplot
  geom_col() +
  # flip coordinates
  coord_flip() +
  # facet by year 2021 and 2022
  facet_wrap(~Year, scales = "free_y") +
  # rename the axis titles and plot title
  labs(
    x = "State",y = "Population (in Millions)",title = "Population by State"
  ) +
  # formatting
...
# dataframe containing the key regions
regions <- 
  # extract the json file from the url
  fromJSON(url)|>
  # unnest the states column values
  unnest(states) |>
  # rename the state name column
  rename(state_name = states) |>
  # only extract the relevant state names
  filter(state_name %in% c(state.name, "District of Columbia", "Puerto Rico")) |>
  # change the name is the region with the long name
  mutate(region_name = case_when(
    region_name == "New York and New Jersey, Puerto Rico, Virgin Islands" ~ "Coastal Territories",
    TRUE ~ region_name
  ))
...
# join by state_name to add the region and region name column to population
population <- population |>
...
# extract into the dataframe
cases_raw <- resp_body_json(response, simplifyVector = TRUE, simplifyDataFrame = TRUE)

# All the data is not there because the API only provides us with a subset of the
# data when we use the default request function. 
...
cases |>
  # extract the year and month for each state
   mutate(date = ymd(date),
         year = year(date),
         month = month(date, label = TRUE, abbr = FALSE)) |>
  # filter years of interest
  filter(year %in% c(2020,2021)) |>
  # obtain the population of each state for each year
  left_join(
    population |> select(state, Year, pop, region_name), 
    by = c("state" = "state", "year" = "Year")
  ) |>
  # cases per 100,000
  mutate(cases_per_100000 = round(cases/pop * 100000,2)) |>
  filter(!is.na(cases_per_100000)) |>
  # cases per 100,000 versus date for each state
  ggplot(aes(x = date, y = cases_per_100000, group = state, color = state)) +
  geom_line(alpha = 0.6) +
  # stratify the plot by region name
  facet_wrap(~ region_name, scales = "free_x") +
  labs(
    title = "SARS COV-2 Cases per 100,000 by Region",
    x = "Date",
    y = "Cases per 100,000"
  ) +
theme(
...
kable(cases |> 
  # use lubridate package to parse the date column
  mutate(date = ymd(date),
         year = year(date),
         month = month(date, label = TRUE, abbr = FALSE)) |>
  # group by each state
  group_by(year, month) |>
  # only years 2020 and 2021
  filter(year %in% c(2020,2021)) |>
  # total cases by each state
...
deaths <- 
  ...
  resp_body_json(response, simplifyVector = TRUE, simplifyDataFrame = TRUE) |>
  # select relevant columns
  select(c(state,end_date,covid_19_deaths)) |>
  # rename 2 columns
...
deaths |>
  ...
  # get the 10 highest deaths
  slice_head(n = 10) |>
  # x = state, y = total deaths
  ggplot(aes(x = reorder(state, -total_deaths), y = total_deaths/1000000)) +
  # bars
  geom_col() +
  # labels
  labs(
    x = "State",y = "Total Deaths (in Millions)",title = "Total Deaths by State"
  ) +
  # formatting
  theme(plot.title = element_text(hjust = 0.5),
        axis.text.x = element_text(angle = 45, hjust = 1))
